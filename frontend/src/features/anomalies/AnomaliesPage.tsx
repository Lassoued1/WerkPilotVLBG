...
import { defaultAnomalyFilters, fetchAnomalies, fetchAnomaly, rerunAnomalies, updateAnomalyStatus, type Anomaly, type AnomalyFilters, type AnomalyStatus } from "./api";

const managerRoles = ["ADMIN", "PRODUCTION_MANAGER", "ENERGY_MANAGER"];
const statusLabels: Record<AnomalyStatus, string> = { NEW: "Neu", ACKNOWLEDGED: "Bestätigt", IN_PROGRESS: "In Bearbeitung", RESOLVED: "Gelöst", DISMISSED: "Verworfen", SUPERSEDED: "Ersetzt" };
...
export function AnomaliesPage() {
  ...
  const statusMutation = useMutation({ mutationFn: ({ id, status }: { id: string; status: AnomalyStatus }) => updateAnomalyStatus(id, status), onSuccess: () => void queryClient.invalidateQueries({ queryKey: ["anomalies"] }) });
  const rerunMutation = useMutation({ mutationFn: () => rerunAnomalies(filters), onSuccess: () => void queryClient.invalidateQueries({ queryKey: ["anomalies"] }) });
  ...
  return <section className="page-stack" aria-labelledby="anomalies-title">
    ...
    {selectedId ? <AnomalyDetails detail={detailQuery.data} error={detailQuery.error} isLoading={detailQuery.isPending} /> : null}
  </section>;
}
...
function AnomalyRow({ anomaly, canManage, onDetails, onStatus }: { anomaly: Anomaly; canManage: boolean; onDetails: () => void; onStatus: (status: AnomalyStatus) => void }) {
  return <tr><th scope="row">{formatDateTime(anomaly.periodStart)}</th><td>{anomaly.anomalyType}</td><td>{anomaly.severity === "CRITICAL" ? "Kritisch" : "Warnung"}</td><td>{statusLabels[anomaly.status]}</td><td>{anomaly.machineId ?? anomaly.lineId ?? "-"}</td><td className="action-cell"><button className="secondary-button" onClick={onDetails} type="button">Details</button>{canManage && anomaly.status !== "SUPERSEDED" ? <select aria-label={`Status für ${anomaly.id}`} defaultValue={anomaly.status} onChange={(event) => onStatus(event.target.value as AnomalyStatus)}><option value={anomaly.status}>{statusLabels[anomaly.status]}</option>{(["ACKNOWLEDGED", "IN_PROGRESS", "RESOLVED", "DISMISSED"] as AnomalyStatus[]).filter((status) => status !== anomaly.status).map((status) => <option key={status} value={status}>{statusLabels[status]}</option>)}</select> : null}</td></tr>;
}

function AnomalyDetails({ detail, error, isLoading }: { detail?: Awaited<ReturnType<typeof fetchAnomaly>>; error: unknown; isLoading: boolean }) {
